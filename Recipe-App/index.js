...
function searchMeal() {
    ...
    fetch(apiUrl + userInput).then((response) => response.json()).then((data) => {
        ...
        const recipeHtml = `
            ...
            <button id="another-recipe">Another Recipe</button>
        `;
        // Assuming you have the 'meal.strMealThumb' variable with the image URL
...
overlayDiv.style.backgroundColor = "rgba(0, 0, 0, 0.5)"; // Dark overlay with 0.5 opacity

// Set the background image and styles for the body element
bodyElement.style.backgroundImage = `url('${mealImageURL}')`;
...
bodyElement.appendChild(overlayDiv);
        ...
        const anotherRecipeBtn = document.getElementById("another-recipe");
        anotherRecipeBtn.addEventListener("click", newRecipeBtn);
    })
        // Handle error
        .catch(() => {
